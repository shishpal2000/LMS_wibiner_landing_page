...
const FAQs: React.FC<FAQsProps> = ({ faqs }) => {
  // console.log("FAQs Component - Received data:", faqs);

  const [openFAQ, setOpenFAQ] = useState<number | null>(null);

  // Unwrap the nested array structure
  const faqList = faqs?.[0] ?? []; // Access the inner array safely
  ...
  const handleSubmit = async (
    values: { name: string; email: string; number: string; query: string },
              ...
              <div>
                {faq.content?.length > 0 ? (
                  faq.content?.map((item, index) => (
                    <div
                      key={index}
                      className="flex flex-col gap-4 mb-4 text-gray-700"
                    >
                ...
                >
                  {({ isSubmitting }) => (
                    <Form className="w-full md:w-[100%] flex flex-col p-[3%] gap-3 bg-gray-100 rounded-md h-fit mb-24 md:md-10 lg:mb-0">
                      ...
                      <div className="w-full flex flex-col gap-1">
                        <p className="text-xs lg:text-xs xl:text-sm">
                          Mobile Number
                        </p>
                        <Field
                          name="number"
